...
async def test_create_topic(git_adapter_sync, test_user):
    ...
    topic = Topic(
        id="test_topic",
        name="Test Topic",
        metadata={"description": "Test topic", "created": "2024-01-01"}
    )
    
    await adapter.create_topic(topic)
    
    topic_dir = adapter.repo_path / "topics" / "Test Topic"
    assert topic_dir.exists()
    assert topic_dir.is_dir()
    assert (topic_dir / "messages.jsonl").exists()
    assert (topic_dir / "media").exists()
...
async def test_save_attachment(git_adapter):
    ...
    topic_dir = git_adapter.repo_path / "topics" / "Test Topic"
    attachments_dir = topic_dir / "media" / "jpg"
    assert attachments_dir.exists()
    assert (attachments_dir / "test.jpg").exists()
...
async def test_save_multiple_messages(git_adapter):
    ...
    messages = [
        Message(content=f"Message {i}", source="test", timestamp=datetime.utcnow())
        for i in range(3)
    ]
...
async def test_save_binary_attachment(git_adapter):
    ...
    topic_dir = git_adapter.repo_path / "topics" / "Test Topic"
    attachments_dir = topic_dir / "media" / "bin"
    assert (attachments_dir / "test.bin").exists()
